...
if (document.querySelector('.puy')) {
	document.querySelector('.puy').addEventListener('click', function () {
		...
		console.log(document.querySelectorAll('.insurance-block-add-subinsurance'))
		console.log(document.querySelectorAll('.insurance-block-add'))
...
function collectInsurance() {
	...
	inputBlocks.forEach((block, index) => {
		...
		if (name == '' || selectedOption == '' || placeOfBirth == '' || iin == '' || dateOfBirth == '')  {
			...
			toastBody.innerHTML = 'Заполните все поля формы'
			BsAlert.show();
		}else{
...
var SomePolis = document.querySelector('.some-block-polis')
if (SomePolis){
	SomePolis.addEventListener('click', function (e) {
		validateForm()
	})
}
...
function validateForm() {
  var form = document.querySelector('.block__insurance-form1');
  var isValid = form.checkValidity();

  if (isValid) {
    alert("Ваша форма отправлена успешно!");
		...
		function saveLocale() {
			if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
				...
				if (cookie === 'kz') {
					...
					changeLocale('lat')
					document.querySelector('.nav__dropdown-flag').src = 'img/kz.png'
					document.querySelector('.nav__dropdown-text').innerHTML = 'Qazaqşa'
				} else if (cookie === 'en') {
					changeLocale('en')
